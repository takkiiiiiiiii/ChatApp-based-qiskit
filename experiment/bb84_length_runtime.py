...
# For the time being, currently, the shifted keys are compared with each other in a single function to generate a share key. (Only eliminating error bit positions in the comparison).
    ...
    runtime = end - start
    return runtime, alice_sharekey, len(alice_sharekey)
...
def main():
    ...
    key_length = 0
    total_keylength = ''
    while (len(total_keylength) < len_key):
        runtime, key, key_length = bb84(user0, user1, num_qubit_linux, len_key)
        total_keylength += key
        print(f'Current key length: {total_keylength}')
        print("Running time to generate share key: ", runtime, "\n\n")
        total_runtime += runtime
    print(f'Total runtime to generate the sharekey ({count} times); {total_runtime}\n\n')
